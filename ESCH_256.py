...
def ESCH_256(eng, M_value, M_len):
    ...
    for i in range(l-1):
        M3(eng, M[128*i: 128*i+128], S)
        Sparkle(eng, S, carry, 7)
    ...
    Sparkle(eng, S, carry, 11)

    result = eng.allocate_qureg(128)

    for i in range(128):
        CNOT | (S[383-i], result[127-i])

    All(Measure) | result
    print('\nD0\n')
    for i in range(128):
        print(int(result[127 - i]), end='')
    print('\n')
...
def M3(eng, M, S):
    ...
    XOR_32(eng, x0, S[352:384])
    XOR_32(eng, y0, S[320:352])
    XOR_32(eng, x1, S[288:320])
    XOR_32(eng, y1, S[256:288])
...
def Sparkle(eng, x, carry, number):
    ...
    for i in range(8):
        ...
        y4.append(x[16 + i + 64])
        x4.append(x[16 + i + 96])
        y3.append(x[16 + i + 128])
        x3.append(x[16 + i + 160])
        y2.append(x[16 + i + 192])
        x2.append(x[16 + i + 224])
        y1.append(x[16 + i + 256])
        x1.append(x[16 + i + 288])
        y0.append(x[16 + i + 320])
        x0.append(x[16 + i + 352])
    for i in range(8):
        y5.append(x[24 + i])
        x5.append(x[24 + i + 32])
        y4.append(x[24 + i + 64])
        x4.append(x[24 + i + 96])
        y3.append(x[24 + i + 128])
        x3.append(x[24 + i + 160])
        y2.append(x[24 + i + 192])
        x2.append(x[24 + i + 224])
        y1.append(x[24 + i + 256])
        x1.append(x[24 + i + 288])
        y0.append(x[24 + i + 320])
        x0.append(x[24 + i + 352])
    ...
    for i in range(number):
        ...
        AC(eng, x0, y0, carry, c[0])
        AC(eng, x1, y1, carry, c[1])
        AC(eng, x2, y2, carry, c[2])
        AC(eng, x3, y3, carry, c[3])
        AC(eng, x4, y4, carry, c[4])
        AC(eng, x5, y5, carry, c[5])

        L6(eng, x0, x1, x2, x3, x4, x5, y0, y1, y2, y3, y4, y5)
...
def L6(eng, x0, x1, x2, x3, x4, x5, y0, y1, y2, y3, y4, y5):

    with Compute(eng):
        XOR_32(eng, y0, y2)
        XOR_32(eng, y1, y2)
        XOR_16(eng, y2[0:16], y2[16:32]) # a+b || b

    XOR_16(eng, y2[16:32], x4[0:16])
    XOR_16(eng, y2[0:16], x4[16:32])
    XOR_32(eng, x1, x4)

    XOR_16(eng, y2[16:32], x5[0:16])
    XOR_16(eng, y2[0:16], x5[16:32])
    XOR_32(eng, x2, x5)

    XOR_16(eng, y2[16:32], x3[0:16])
    XOR_16(eng, y2[0:16], x3[16:32])
    XOR_32(eng, x0, x3)
    ...
    with Compute(eng):
        XOR_32(eng, x0, x2)
        XOR_32(eng, x1, x2)
        XOR_16(eng, x2[0:16], x2[16:32])  # a+b || b

    XOR_16(eng, x2[16:32], y4[0:16])
    XOR_16(eng, x2[0:16], y4[16:32])
    XOR_32(eng, y1, y4)

    XOR_16(eng, x2[16:32], y5[0:16])
    XOR_16(eng, x2[0:16], y5[16:32])
    XOR_32(eng, y2, y5)

    XOR_16(eng, x2[16:32], y3[0:16])
    XOR_16(eng, x2[0:16], y3[16:32])
    XOR_32(eng, y0, y3)
...
(ESCH_256(eng, 0x201f1e1d1c1b1a191817161514131211100f0e0d0c0b0a09080706050403020100, 32))
eng.flush()
